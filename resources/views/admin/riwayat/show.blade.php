<x-app-layout>
    ...
    @endif
    <x-card title="Berikut hasil diagnosa kulit">
        <p class="mb-4">
            <i class="fas fa-user mr-1"></i> {{ $riwayat->nama }} <i class="fas fa-calendar ml-4 mr-1"></i>
            {{ $riwayat->created_at->format('d M Y, H:m:s') }}
        </p>
        ...
        @foreach (unserialize($riwayat->hasil_diagnosa) as $diagnosa)
            <div class="card card-body p-0 mt-5 border" style="box-shadow: none !important;">
                <div class="card-header bg-primary text-white p-2">
                    <h6 class="font-weight-bold">Tabel perhitungan kulit: {{ $diagnosa['nama_kulit'] }}
                        ({{ $diagnosa['kode_kulit'] }})</h6>
                </div>
        ...
        <div class="mt-5">
            <div class="alert alert-success">
                ...
                <p>Berdasarkan dari gejala yang kamu pilih atau alami juga berdasarkan Role/Basis aturan yang sudah
                    ditentukan oleh seorang pakar Kulit maka perhitungan Algoritma Certainty Factor mengambil nilai
