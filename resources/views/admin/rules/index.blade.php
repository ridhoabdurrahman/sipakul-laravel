<x-app-layout>
    ...
    <x-card>
        <div class="row">
            <div class="col-md-8">
                <form action="{{ route('admin.rules.update', $data_kulit->id) }}" method="POST" id="update_rules">
                    @csrf
                    <div class="table-responsive">
                        <table class="table align-items-center table-flush table-hover border" id="dataTableHover">
                            ...
                            <tbody>
                                ...
                                @foreach ($gejala_kulit->get() as $row)
                                    <tr>
                                        <td>{{ $rows }}</td>
                                        <td>{{ $row->kode }}</td>
                                        <td>{{ $row->nama }}</td>
            ...
            <div class="col-md-4">
                <div class="list-group">
                    @foreach ($kulit as $row)
                        <a href="{{ route('admin.rules', $row->id) }}"
                            class="list-group-item list-group-item-action {{ $data_kulit->nama == $row->nama ? 'active' : '' }}">
                            {{ $row->nama }}
                        </a>
                    @endforeach
                </div>
