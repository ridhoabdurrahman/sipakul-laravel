...
use App\Models\{Gejala, Kulit, Rule, Riwayat};
...
class DiagnosaController extends Controller
{
    ...
    public function kalkulasi_cf($data)
    {
        ...
        foreach ($data['diagnosa'] as $input) {
            if (!empty($input)) {
                $opts = explode('+', $input);
                $gejala = Gejala::with('kulits')->find($opts[0]);

                foreach ($gejala->kulits as $kulit) {
                    if (empty($data_kulit[$kulit->id])) {
                        $data_kulit[$kulit->id] = [$kulit, [$gejala, $opts[1], $kulit->pivot->value_cf]];
                    } else {
                        array_push($data_kulit[$kulit->id], [$gejala, $opts[1], $kulit->pivot->value_cf]);
                    }
            ...
            foreach ($final as $key => $value) {
                ...
                if (empty($hasil_diagnosa[$final[0]->id])) {
                    $hasil_diagnosa[$final[0]->id] = [
                        'nama_kulit' => $final[0]->nama,
                        'kode_kulit' => $final[0]->kode,
                        'gejala' => [
                            [
